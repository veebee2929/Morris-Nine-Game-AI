...
def minmaxgameblack(pos, next, depth):
  global static_pos
  ...
  if next :
    ...
    for item in prob_open:
      curr = minmaxgameblack(pos = item,  next = False, depth = depth - 1)
    ...
    for item in prob_open:
      curr = minmaxgameblack(pos = item,  next = True, depth = depth - 1)
...
for i in L :
  ...
  curr = minmaxgameblack(pos = i, next= True, depth = depth - 1)
